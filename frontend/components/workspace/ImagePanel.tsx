...
// Extracted to its own component so useState/useEffect work cleanly
function ImageContent({
  ...
  loading: boolean;
  emptyState: boolean;
  emptyContext?: { frameA?: string; frameB?: string; title?: string; message?: string };
  onZoom?: () => void;
  downloadUrl?: string | null;
}) {
  ...
  return (
    <div className="grow relative flex items-center justify-center bg-space-navy-950/20 overflow-hidden">
      {emptyState ? (
        ...
        /* Standby State — no image, or 404 error */
        <div className="flex flex-col items-center justify-center space-y-2 text-center select-none px-4">
          <div className="h-8 w-8 rounded-full border border-space-navy-700 bg-space-navy-900 flex items-center justify-center">
          ...
          {imgError && (
            <span className="text-[9px] font-mono text-slate-700">
              Frame not available for this timestamp
            </span>
          )}
        </div>
      )}
